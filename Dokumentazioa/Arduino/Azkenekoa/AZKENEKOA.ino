...
void startAccessPoint() {
  ...
  void connectToWiFi() {
      ...
      if (WiFi.status() == WL_CONNECTED) {
          ...
          // 🔹 Mostrar la IP en la pantalla por unos segundos
          mostrarIPenPantalla(newIP);
      } else {
...
//====================================================================================
void mostrarIPenPantalla(String ip) {
    digitalWrite(digit5, PIN_LOW);  // Activar la pantalla para mostrar la IP
    tft.fillScreen(TFT_BLACK);      // Limpiar la pantalla
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.setTextSize(2);
    tft.setCursor(10, 50);          // Ajustar posición del texto
    tft.print("IP:");
    tft.setCursor(10, 80);
    tft.print(ip);                  // Mostrar la IP en pantalla
    delay(5000);                     // Mantener la IP visible por 5 segundos
    digitalWrite(digit5, PIN_HIGH);  // Apagar la pantalla después del tiempo
}
